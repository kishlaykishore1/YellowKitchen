...
extension SceneDelegate {
    ...
    func isUserLogin(_ isLogin: Bool) {
        if isLogin {
            let homeTabBar = StoryBoard.Home.instantiateViewController(withIdentifier: "HomeVC") as! HomeVC
            let nav = UINavigationController(rootViewController: homeTabBar)
            //nav.isNavigationBarHidden = true
            window?.rootViewController = nav
            window?.makeKeyAndVisible()
        } else {
            let welcomeVC = StoryBoard.Main.instantiateViewController(withIdentifier: "MyAccountVC") as! MyAccountVC
